CREATE TABLE IF NOT EXISTS notifications (
    ...
    status VARCHAR(20) NOT NULL DEFAULT 'pending' CHECK (status IN ('pending', 'delivered', 'read', 'expired')),
...
CREATE INDEX idx_outbox_notification_id ON outbox_events(notification_id);
CREATE INDEX idx_outbox_user_status ON outbox_events(user_id, status);
